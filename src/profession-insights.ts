...
const factorNames={coverage:'近期招聘机会',directEntry:'本科毕业能做的工作',schoolAccess:'按你位次可选的学校',stability:'工作需求的稳定程度'} as const
...
export function buildProfessionInsights(card:ProfessionCard){
  ...
  return [
    {label:'为什么放在这组',text:`这个专业被放在“${card.band}”${strongest?`，主要因为“${factorNames[strongest[0]]}”这一项表现较好（${strongest[1].value} 分）。${strongest[1].evidence}。`:'，但目前能查到的资料还不够。'}`},

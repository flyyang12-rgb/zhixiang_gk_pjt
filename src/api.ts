...
export type PlanningCoordinate={rank:number|null;sampleCount:number;bestRank:number|null;worstRank:number|null;spreadRatio:number|null;stability:'single'|'preliminary'|'stable'|'moderate'|'volatile'}
export type RecommendationResult = { generatedAt: string; sourceYear: number|null; dataYears?: number[]; sources?: RecommendationSource[]; candidates: RecommendationCandidate[]; planningCoordinate?:PlanningCoordinate;warning: string }

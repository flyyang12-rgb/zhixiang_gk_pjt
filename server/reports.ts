...
function renderReport(data: { profile: RowDataPacket; recommendation: RowDataPacket|null }) {
  ...
  const planningRank=recommendation.planningCoordinate?.rank??Number(data.profile.provinceRank||0)

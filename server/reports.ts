...
function renderReport(data: { profile: RowDataPacket; recommendation: RowDataPacket|null }) {
  const recommendation = parse<{sourceYear:number|null;dataYears?:number[];warning:string;sources?:Array<{title:string;sourceUrl:string;sourceYear:number;publisher:string}>;candidates:Array<{schoolName:string;province:string;city:string;level:string;risk:string;referenceRank:number;ruleScore:number;majors:Array<{name:string;fit:string}>}>}>(data.recommendation?.result) ?? { sourceYear:null, warning:'尚未生成候选清单', candidates:[] }
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><style>${reportCss}</style></head><body><header><i>知</i><div><h1>高考志愿规划报告</h1><p>家长主导 · 学生共同参与 · 本地隐私分析</p></div><span>${new Date().toLocaleDateString('zh-CN')}</span></header><section class="hero"><div><small>学生档案</small><h2>${safe(data.profile.studentName)}</h2><p>${safe(data.profile.province)} · ${safe(data.profile.subjectGroup)}</p></div><strong>${data.profile.score}<small>分</small></strong><strong>${Number(data.profile.provinceRank||0).toLocaleString()}<small>位次</small></strong></section><section><h3>01 院校 / 专业候选</h3><p class="note">实际使用年份：${recommendation.dataYears?.join('、')??recommendation.sourceYear??'暂无'}。${safe(recommendation.warning)}</p>${rows?`<table><thead><tr><th>#</th><th>学校</th><th>层级</th><th>参考位次</th><th>规则评分</th><th>专业方向</th></tr></thead><tbody>${rows}</tbody></table>`:'<div class="empty">本省官方数据尚未导入，暂不生成冲稳保。</div>'}</section><section><h3>02 数据来源</h3>${sources?`<ul class="sources">${sources}</ul>`:'<p>暂无可列出的招生数据来源。</p>'}</section><footer><b>不确定性与重要说明</b><p>本报告仅作家庭讨论与信息整理，不构成录取承诺。往年位次会受招生计划、专业组调整与考生偏好变化影响；正式填报前，请以当年省级教育考试院招生计划、选科要求和院校招生章程为准。学生兴趣与执行意愿必须进入最终决策。</p></footer></body></html>`
}

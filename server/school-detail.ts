...
export async function loadSchoolDetail(schoolId:number,profileId?:string) {
  ...
  const [majorRows]=await database.query<RowDataPacket[]>(
    `SELECT fme.id evidenceId,m.id majorId,COALESCE(m.name,fme.major_name) name,m.category,fme.major_code majorCode,fme.education_level educationLevel,fme.recognition_type recognitionType,fme.recognition_year recognitionYear,
  ...
  if(profileId){
    ...
    const [recordRows]=await database.query<RowDataPacket[]>(
      `SELECT ap.id,ap.year,ap.education_level educationLevel,ap.admission_category admissionCategory,ap.batch,ap.plan_type planType,
       ap.eligibility_requirement eligibilityRequirement,ap.recommendation_eligible recommendationEligible,
       ap.recommendation_exclusion_reason recommendationExclusionReason,ap.unit_type unitType,ap.major_name unitName,ap.unit_code unitCode,
  ...
  const interpretation=[
    ...
    {label:'核验提醒',text:school.admissionsUrl?'正式填报前仍需到招生官网核对当年招生计划、选科要求和院校章程。':'该校招生官网尚未完成核验，当前仅展示数据库中已有事实。'},
  ]

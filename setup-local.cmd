...
echo [2/9] 初始化本地 MySQL...
call npm run db:init || goto :error
echo [3/9] 导入教育部全国高校名单...
call npm run data:schools || goto :error
echo [4/9] 导入已核验院校官网与招生网...
call npm run data:school-links || goto :error
echo [5/9] 导入已核验优势专业证据...
call npm run data:featured-majors || goto :error
echo [6/9] 导入已核验专业未来发展证据...
call npm run data:major-outlook || goto :error
echo [7/9] 导入山东 2023-2025 官方投档数据...
call npm run data:shandong || goto :error
echo [8/9] 导入河南 2023-2025 投档数据...
call npm run data:henan || goto :error
echo [9/9] 导入河北 2023-2025 投档数据...

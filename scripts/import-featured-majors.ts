  ...
  for(const [index,value] of values.entries()){
    try{
      ...
      try{
        ...
        await connection.execute(`INSERT INTO data_sources(source_type,title,source_url,source_year,publisher,published_at,collected_at) VALUES('major',?,?,?,?,NULL,?) ON CONFLICT (source_url,source_year) DO UPDATE SET title=EXCLUDED.title,publisher=EXCLUDED.publisher,collected_at=EXCLUDED.collected_at`,[`${record.schoolName}${record.recognitionType}`,record.sourceUrl,record.sourceYear,record.publisher,new Date(record.verifiedAt)])
        ...
        const [existingRows]=await connection.query<RowDataPacket[]>('SELECT id FROM school_featured_major_evidence WHERE school_id=? AND major_name=? AND recognition_type=? AND recognition_year IS NOT DISTINCT FROM ? LIMIT 1',[schools[0]!.id,record.majorName,record.recognitionType,record.recognitionYear??null])

...
try{
  const [provinceRows]=await connection.query<DatabaseRow[]>(`SELECT id FROM provinces WHERE name='河南'`)
  ...
  for(const subjectGroup of ['物理类','历史类'] as const){
    ...
    const [sourceResult]=await connection.execute<DatabaseResult>(
      `INSERT INTO data_sources(source_type,title,source_url,source_year,publisher,published_at) VALUES('admission',?,?,2025,?,'2025-07-19') ON CONFLICT (source_url,source_year) DO UPDATE SET title=EXCLUDED.title,publisher=EXCLUDED.publisher RETURNING id`,
  ...
  for(const legacyConfig of [
    ...
    const [sourceResult]=await connection.execute<DatabaseResult>(`INSERT INTO data_sources(source_type,title,source_url,source_year,publisher,published_at) VALUES('admission',?,?,?,?,?) ON CONFLICT (source_url,source_year) DO UPDATE SET title=EXCLUDED.title,publisher=EXCLUDED.publisher,published_at=EXCLUDED.published_at RETURNING id`,[legacyConfig.title,legacyConfig.sourceUrl,legacyConfig.year,legacyConfig.publisher,legacyConfig.publishedAt])

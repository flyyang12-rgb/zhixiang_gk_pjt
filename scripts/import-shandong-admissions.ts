...
try{
  const [provinceRows]=await connection.query<DatabaseRow[]>(`SELECT id FROM provinces WHERE name='山东'`)
  ...
  for(const source of sources){
    ...
    const [sourceResult]=await connection.execute<DatabaseResult>(
      `INSERT INTO data_sources(source_type,title,source_url,source_year,publisher,published_at) VALUES('admission',?,?,?,'山东省教育招生考试院',?) ON CONFLICT (source_url,source_year) DO UPDATE SET title=EXCLUDED.title,publisher=EXCLUDED.publisher,published_at=EXCLUDED.published_at RETURNING id`,
